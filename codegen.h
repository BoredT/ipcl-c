...
public:
    ...
    Value* getVar(std::string name) {
        std::stack<CodeGenBlock *> copy = blocks;
        while (!copy.empty()) {
            if (copy.top()->locals.find(name) == copy.top()->locals.end()) { //not found in last block
                copy.pop();
            }
            else {
                return copy.top()->locals[name];
            } 
        }
